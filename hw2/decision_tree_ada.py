...
def info_gain(data, split_attr, target_attr, gain_method, weights):
    ...
    total_e = gain_method(data[target_attr], weights)
    ...
    for i in range(len(vals)):
        split_data = data.where(data[split_attr] == vals[i]).dropna()[
            target_attr]
        new_e += (freqs[i]/np.sum(freqs))*gain_method(split_data, weights)
    ...
    # if there are no more attributes, return the parent label
        ...
        for val in np.unique(original_data[best_attr]):
            ...
            new_tree = id3(new_data, original_data, attrs, target_attr,
                           gain_method, parent_label, weights, current_depth, max_depth)
